...
function problemC () {
  ...
  // ???
    promisifiedReadFile('poem-one/stanza-02.txt').then((stanza) => {
        blue(stanza);
...
function problemD () {
  ...
  // promise version
    promisifiedReadFile('poem-one/stanza-04.txt').then((stanza) => blue(stanza)).catch((err)=> magenta(err));
}
...
function problemE () {
  ...
  // promise version
  // ???
    promisifiedReadFile('poem-one/stanza-03.txt').then((stanza) => {
        blue(stanza)
        return promisifiedReadFile('poem-one/stanza-04.txt');
    }, (err) => magenta(err)).then((stanza) => blue(stanza), (err) => magenta(err));
...
function problemF () {
  ...
  // promise version
    promisifiedReadFile('poem-one/stanza-03.txt').then((stanza) => {
        blue(stanza)
        return promisifiedReadFile('poem-one/stanza-04.txt')}).then((stanza) => {
        blue(stanza)
        console.log('done')
    }, (err) => magenta(err));
};
